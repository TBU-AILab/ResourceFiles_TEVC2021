...
void searchAlgorithm::modifySolutionWithParentMedium(Individual child, Individual parent) {
  ...
  //kontrola hranic je zde
  if(tom_bound == 0 || tom_bound == 5) {
  ...
  if(tom_bound == 1) {
	  ...
	  if(tom_bound == 2) {
		for (int i = 0; i < num ; i++) {
			if (child[i] > max) {
				child[i] = ((max_region - min_region) * randDouble()) + min_region;;
			}
			else if (child[i] < min)
			{
				child[i] = ((max_region - min_region) * randDouble()) + min_region;; 
	  ...
	  if(tom_bound == 4) {
		...
		for (int i = 0; i < num ; i++) {
			if (child[i] > max) {
				child[i] = max - (child[i] - max);
			}
			else if (child[i] < min)
			{
				child[i] = min + (min - child[i]);
			}
		}
		return ;
	  } 
