...
const int nRuns=30;      // number of runs (30 in CEC 2020)
double REZ[4][nFun][nRuns];   // data for Tables II-V   4...D=5,10,15,20

const int rawNum=16; // number of rawdata values
double rawData[4][nFun][nRuns][rawNum];
const string DIRNAME="rawResults";              // directory for files of raw results
...
void genTablesRawData(const double rawData[4][nFun][nRuns][rawNum]) {
   for (int D = 5; D <= 20; D=D+5) { // CEC2020 dimensions 5,10,15,20
      for (int iFunk=0; iFunk<nFun; iFunk++) {
         ostringstream ss;
         ss << DIRNAME<<"/j2020_"<<iFunk+1<<"_"<<D<<".txt";  // for example, j2020_1_5.txt, ...
         string s = ss.str();
         ...
         for (int k=0; k<rawNum; k++) {  // for each raw
            for (int iRun=0; iRun<nRuns; iRun++) {  // for each RUN
               outFile << rawData[D/5-1][iFunk][iRun][k]<<" ";
            }
            outFile << endl;
         }
         outFile.close();
...
void genTableREZ(const double REZ[4][nFun][nRuns]) {
   for (int D = 5; D <= 20; D=D+5) { // CEC2020 dimensions 5,10,15,20
      ostringstream ss;
      ss << "table"<<D<<"D.tex";  // table5D.tex, ...
      string s = ss.str();
      ofstream outFile(s.c_str());
      ...
      for (int iFunk=0; iFunk<nFun; iFunk++) {
         ...
         for (int iRun=0; iRun<nRuns; iRun++) {  // for each RUN
            dataOne[iRun] = REZ[D/5-1][iFunk][iRun];
         }
         // sort
...
//######################################################################################################   
//global variable - BOUNDARY SWITCH
int bound = 0;
//######################################################################################################
...
int main(int argc, char *argv[]) {
   ...
   //######################################################################################################   
   //global variable - BOUNDARY SWITCH - read
   // cout << argv[1];  // bound 
   bound = atoi(argv[1]);
   //cout << endl << "\t" << bound << endl << endl;
   //return 0;
   //######################################################################################################
   ...
   //######################################################################################################
   //    FOR TESTING
   //for (int D = 5; D <= 5; D=D+5) { // CEC2020 dimensions 5,10,15,20
   for (int D = 5; D <= 20; D=D+5) {
   //######################################################################################################
      ...
      switch (D) {                   // CEC 2020
         case  5: maxFES =    50000; break; 
         case 10: maxFES =  1000000; break; 
         case 15: maxFES =  3000000; break; 
         case 20: maxFES = 10000000; break; 
      }

      for (int iFunk = 0; iFunk < nFun; iFunk++) {  // for each FUNCTION
         double optimum;
         switch (iFunk+1) {            // CEC 2020
         ...
         for (int iRun=0; iRun<nRuns; iRun++) {  // for each RUN
            ...
            for (int k=0; k<rawNum; k++)
               rawData[D/5-1][iFunk][iRun][k] = -1; // -1 means no data
            ...
            for (int i=0; i<NP; i++) {
               ...
               for (int k=0; k<16; k++) {
                  if (i+1 == (int)(pow(D,(k/5.0 - 3.0)) * maxFES)) {
                     rawData[D/5-1][iFunk][iRun][k] = cost[indBest];
            ...
            for (it=NP; it < maxFES; it++) {  // in initialization, NP function evaluations
               ...
               for(int j=0; j<D; j++) {    // mutation and crossover
                  if (mRandom() < CR || j == jrand) {
                     ...
                     if(bound == 0) {
                        ...
                        if(bound == 2) {
                           //RANDOM
                           if(U[j] < Xmin || U[j] > Xmax) { U[j] = Xmin + mRandom()*(Xmax - Xmin); }  
                        }
                        if(bound == 3) {
                           //PERIODIC
                           U[j] = Xmin + PSO_mod(U[j] - Xmax, Xmax - Xmin);
                        }
                        if(bound == 4) {
                           //REFLECTION
                           if(U[j] > Xmax) {
               ...
               if (c < cost[indBest]) {   // best  (min)
                  ...
                  if(cost[indBest] < terminateErrorValue) {   // ZERO -- FINISH?
                    ...
                    for (int k=0; k<16; k++) {
                       if(rawData[D/5-1][iFunk][iRun][k] == -1) rawData[D/5-1][iFunk][iRun][k] = 0;  // if we have solve problem before maxFES, we fill rawData with zeros
                     }
                     break;
                  }
               }
               ...
               for (int k=0; k<16; k++) {
                  if (it+1 == (unsigned int)(pow(D,(k/5.0 - 3.0)) * maxFES)) {
                     rawData[D/5-1][iFunk][iRun][k] = bestCOST;
                  }
               }
