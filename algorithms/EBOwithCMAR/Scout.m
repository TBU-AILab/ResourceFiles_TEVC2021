...
function[ x, fitx, setting,bestold,bestx,bnd,fitness,current_eval,res_det] = ...
    Scout( x, ~, prob, setting, iter,bestold,bestx,fitness,bnd,xmin,xmax,n,PopSize,current_eval,I_fno,res_det,Printing,Max_FES, BOUND)
...
if current_eval >=handle_limit*Max_FES
    arxvalid =han_boun(arx', xmax, xmin, x,PopSize,2, BOUND,current_eval);
    arxvalid=arxvalid';
else
    arxvalid=arx;
end
...
if  current_eval >=handle_limit*Max_FES
    ...
    setting.xmean =han_boun(setting.xmean', xmax, xmin, x(1,:),1,2, BOUND,current_eval);
