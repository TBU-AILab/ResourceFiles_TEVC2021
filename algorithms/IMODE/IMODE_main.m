...
%%==============================================================================
function [outcome,com_time,SR,avgFE,res_det,bestx]= IMODE_main(run,I_fno, DIM, BOUND)
    ...
    if (current_eval<Par.Max_FES)
        if rand<=Probs(1)
            ...
            [EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, current_eval,res_det,F,cr] = ...
                ...
                archive_freq, Par.xmin, Par.xmax,  Par.n,  PS1,  current_eval, I_fno,res_det,Par.Printing,Par.Max_FES, Par.Gmax, iter,F,cr, BOUND);
        end
    end
    %% ====================== CMA-ES ======================
    if (current_eval<Par.Max_FES)
