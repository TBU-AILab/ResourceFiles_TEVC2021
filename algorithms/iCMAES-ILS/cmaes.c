	       ...
	       const char * filename)
{
  ...
  t->N = dim;
	...
	t->stopTolFun = pow(10,tunee);
	t->stopTolFunHist = pow(10,tunef);
	t->stopTolX = pow(10,tuneg); /* 1e-11*insigma would also be reasonable */
  ...
  if (t->rgInitialStds == NULL) {
    ...
    for (i=0; i<N; ++i){
    	...
    	//t->rgInitialStds[i] = (inrgsigma == NULL) ? 0.3 : inrgsigma[i];
        // printf("t->rgInitialStds[i], %f\n", (inrgsigma == NULL) ? tunec : inrgsigma[i]);
    }
  }
...
readpara_SupplementDefaults(readpara_t *t)
{
  ...
  if (t->lambda < 2){
      //t->lambda = 4+(int)(3*log((double)N));
  	  t->lambda= 4+(int)(tunea*log((double)N));
    }
    if (t->mu == -1) {
  	//t->mu = t->lambda/2;
      t->mu = t->lambda/tuneb;
